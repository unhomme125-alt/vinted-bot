...
interface AuthComponentProps {
  ...
  // Appelé une fois l'écran "succès" affiché (ex: redirection).
  onSuccess?: () => void;
}
...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "EaseMize", mode = 'signup', onModeChange, onSubmit, onSuccess }: AuthComponentProps) => {
  ...
  const doSubmit = async () => {
    ...
    try {
        ...
        setModalErrorMessage(err instanceof Error ? err.message : "Une erreur est survenue");
        setModalStatus('error');
    }
  };
...
useEffect(() => {
    if (modalStatus === 'success') {
        fireSideCanons();
        // Laisse l'écran "Welcome Aboard" visible avant de déclencher la suite (ex: redirection).
        const t = setTimeout(() => onSuccess?.(), 1600);
        return () => clearTimeout(t);
    }
}, [modalStatus, onSuccess]);
