...
interface LoginPageProps {
  // Mode courant : 'login' (connexion) ou 'signup' (inscription).
  mode?: AuthMode;
  // Bascule de mode (lien en bas du formulaire).
  onModeChange?: (mode: AuthMode) => void;
  // Soumission réelle : reçoit (username, password), doit throw une Error en cas d'échec.
  // Absent => flux de démo (mock erik/1234) comme à l'origine.
  onSubmit?: (username: string, password: string) => Promise<void>;
  // Appelé après une soumission réussie (ex: redirection).
  onSuccess?: () => void;
  brandName?: string;
}

function LoginPage({ mode = "login", onModeChange, onSubmit, onSuccess, brandName = "Vinted Bot" }: LoginPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (onSubmit) {
        // Connexion / inscription réelle pilotée par la page.
        await onSubmit(username, password);
        onSuccess?.();
      } else {
        // Pas de handler : flux de démo (mock).
      ...
      <div className="flex items-center justify-center p-8 bg-background">
        <div className="w-full max-w-[420px]">
          ...
          <div className="text-center mb-10">
            ...
            <p className="text-muted-foreground text-sm">{isLogin ? "Please enter your details" : "Choose a username and a password"}</p>
          </div>
